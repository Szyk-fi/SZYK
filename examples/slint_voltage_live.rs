//! Live prototype for Voltage. Same pattern as slint_plaits_live.rs --
//! see slint_mixer_live.rs's doc comment for the fuller "why
//! `#[path]`" explanation. Drives the real `VoltageApp` (a 16-pad
//! analog-style poly synth) through Slint: pads trigger real notes,
...
//!
//! Run it:
//!     cargo run --example slint_voltage_live
...
slint::slint! {
    ...
    export component LiveVoltageScreen inherits DeviceFrame {
        app-name: "CASCADE";
...
fn main() {
    ...
    let mut voltage_app = VoltageApp::new(Arc::clone(&sensitivity), Arc::clone(&nav_speed), Arc::clone(&modbus), Arc::clone(&audio_bus), Arc::clone(&mixer_bus));
    if let Some(p) = voltage_app.audio_processor() {
        engine.add(p);
    }
    voltage_app.on_enter();

    let (audio_host, device_state) = AudioHost::open_default(Arc::clone(&engine)).expect("open default audio output");
    println!("Live Voltage prototype -- output device: {}", device_state.current_output());
    let _audio_host = audio_host;

    let ui = LiveVoltageScreen::new().unwrap();
    let voltage_app = Rc::new(RefCell::new(voltage_app));
    ...
    let app_for_timer = Rc::clone(&voltage_app);
