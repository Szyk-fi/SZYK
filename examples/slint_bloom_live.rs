//! Live prototype for Bloom. Same pattern as slint_mixer_live.rs/
//! slint_plaits_live.rs -- see slint_mixer_live.rs's doc comment for
//! the fuller "why `#[path]`" explanation. Drives the real `BloomApp`
//! and its real granular-cloud voice pool through Slint: left knob
//! scrolls/expands the real menu (Master Clock + one group per
//! shape), right knob edits the selected row.
//!
//! Run it:
//!     cargo run --example slint_bloom_live
...
#[path = "../src/util.rs"]
mod util;
// Flat, sibling modules -- Bloom pulls in Plaits' engine-name/root-
...
mod plaits_layout;
// Bloom's own source refers to `crate::apps::plaits::...` (the real
...
slint::slint! {
    ...
    export component LiveBloomScreen inherits DeviceFrame {
        app-name: "BLOOM";
...
fn main() {
    ...
    let mut bloom_app = BloomApp::new(Arc::clone(&sensitivity), Arc::clone(&nav_speed), Arc::clone(&modbus), Arc::clone(&audio_bus), Arc::clone(&mixer_bus));
    if let Some(p) = bloom_app.audio_processor() {
        engine.add(p);
    }
    bloom_app.on_enter();

    let (audio_host, device_state) = AudioHost::open_default(Arc::clone(&engine)).expect("open default audio output");
    println!("Live Bloom prototype -- output device: {}", device_state.current_output());
    let _audio_host = audio_host;

    let ui = LiveBloomScreen::new().unwrap();
    let bloom_app = Rc::new(RefCell::new(bloom_app));
    ...
    let app_for_timer = Rc::clone(&bloom_app);
