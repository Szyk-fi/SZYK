//! Live prototype for Pams. Same pattern as slint_mixer_live.rs/
//! slint_plaits_live.rs -- see slint_mixer_live.rs's doc comment for
//! the fuller "why `#[path]`" explanation. Drives the real `PamsApp`
...
//!
//! Run it:
//!     cargo run --example slint_pams_live
...
#[path = "../src/util.rs"]
mod util;
// Flat, sibling modules -- Pams pulls in Plaits' engine-name/root-
...
slint::slint! {
    ...
    export component LivePamsScreen inherits DeviceFrame {
        app-name: "PAMS";
...
fn main() {
    ...
    let mut pams_app = PamsApp::new(Arc::clone(&sensitivity), Arc::clone(&nav_speed), Arc::clone(&modbus));
    if let Some(p) = pams_app.audio_processor() {
        engine.add(p);
    }
    pams_app.on_enter();

    let (audio_host, device_state) = AudioHost::open_default(Arc::clone(&engine)).expect("open default audio output");
    println!("Live Pams prototype -- output device: {}", device_state.current_output());
    let _audio_host = audio_host;

    let ui = LivePamsScreen::new().unwrap();
    let pams_app = Rc::new(RefCell::new(pams_app));
    ...
    let app_for_timer = Rc::clone(&pams_app);
