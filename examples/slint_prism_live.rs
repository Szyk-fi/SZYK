//! Live prototype for Prism. Same pattern as slint_mixer_live.rs --
//! see its doc comment for the fuller "why `#[path]`" explanation.
//! Drives the real `PrismApp` through Slint: left knob
//! scrolls/expands the real menu, right knob edits the selected row.
//!
//! Run it:
//!     cargo run --example slint_prism_live
...
slint::slint! {
    ...
    export component LivePrismScreen inherits DeviceFrame {
        app-name: "PRISM";
...
fn main() {
    ...
    let mut the_app = PrismApp::new(Arc::clone(&sensitivity), Arc::clone(&nav_speed), Arc::clone(&modbus), Arc::clone(&audio_bus), Arc::clone(&mixer_bus));
    ...
    println!("Live Prism prototype -- output device: {}", device_state.current_output());
    let _audio_host = audio_host;

    let ui = LivePrismScreen::new().unwrap();
