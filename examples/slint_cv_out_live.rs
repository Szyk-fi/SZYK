//! Live prototype for CV Out. Same pattern as slint_mixer_live.rs --
//! see its doc comment for the fuller "why `#[path]`" explanation.
//! Drives the real `CvOutApp` through Slint: left knob
//! scrolls/expands the real menu, right knob edits the selected row.
//!
//! Run it:
//!     cargo run --example slint_cv_out_live
...
slint::slint! {
    ...
    export component LiveCvOutScreen inherits DeviceFrame {
        app-name: "CV OUT";
...
fn main() {
    ...
    let mut the_app = CvOutApp::new(Arc::clone(&sensitivity), Arc::clone(&nav_speed), Arc::clone(&modbus));
    ...
    println!("Live CvOut prototype -- output device: {}", device_state.current_output());
    let _audio_host = audio_host;

    let ui = LiveCvOutScreen::new().unwrap();
