//! Live prototype for Tape. Same pattern as slint_mixer_live.rs --
//! see its doc comment for the fuller "why `#[path]`" explanation.
//! Drives the real `TapeApp` through Slint: left knob
//! scrolls/expands the real menu, right knob edits the selected row.
//!
//! Run it:
//!     cargo run --example slint_tape_live
...
slint::slint! {
    ...
    export component LiveTapeScreen inherits DeviceFrame {
        app-name: "TAPE";
...
fn main() {
    ...
    let mut the_app = TapeApp::new(Arc::clone(&sensitivity), Arc::clone(&nav_speed), Arc::clone(&modbus), Arc::clone(&audio_bus), Arc::clone(&mixer_bus));
    ...
    println!("Live Tape prototype -- output device: {}", device_state.current_output());
    let _audio_host = audio_host;

    let ui = LiveTapeScreen::new().unwrap();
