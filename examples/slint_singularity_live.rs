//! Live prototype for Singularity. Same pattern as slint_mixer_live.rs --
//! see its doc comment for the fuller "why `#[path]`" explanation.
//! Drives the real `SingularityApp` through Slint: left knob
//! scrolls/expands the real menu, right knob edits the selected row.
//!
//! Run it:
//!     cargo run --example slint_singularity_live
...
use singularity::SingularityApp;
...
slint::slint! {
    ...
    export component LiveSingularityScreen inherits DeviceFrame {
...
fn main() {
    ...
    let mut the_app = SingularityApp::new(Arc::clone(&sensitivity), Arc::clone(&nav_speed), Arc::clone(&modbus), Arc::clone(&audio_bus), Arc::clone(&mixer_bus));
    ...
    println!("Live Singularity prototype -- output device: {}", device_state.current_output());
    let _audio_host = audio_host;

    let ui = LiveSingularityScreen::new().unwrap();
