//! Live prototype for Nebula. Same pattern as slint_mixer_live.rs/
//! slint_plaits_live.rs -- see slint_mixer_live.rs's doc comment for
//! the fuller "why `#[path]`" explanation. Drives the real `NebulaApp`
//! and its real gravity-well particle sim through Slint: left knob
//! scrolls/expands the real menu, right knob edits the selected row.
//!
//! Run it:
//!     cargo run --example slint_nebula_live
...
#[path = "../src/util.rs"]
mod util;
// Flat, sibling modules -- Nebula pulls in Plaits' engine-name/root-
...
slint::slint! {
    ...
    export component LiveNebulaScreen inherits DeviceFrame {
        app-name: "NEBULA";
...
fn main() {
    ...
    let mut nebula_app = NebulaApp::new(Arc::clone(&sensitivity), Arc::clone(&nav_speed), Arc::clone(&modbus), Arc::clone(&audio_bus), Arc::clone(&mixer_bus));
    if let Some(p) = nebula_app.audio_processor() {
        engine.add(p);
    }
    nebula_app.on_enter();

    let (audio_host, device_state) = AudioHost::open_default(Arc::clone(&engine)).expect("open default audio output");
    println!("Live Nebula prototype -- output device: {}", device_state.current_output());
    let _audio_host = audio_host;

    let ui = LiveNebulaScreen::new().unwrap();
    let nebula_app = Rc::new(RefCell::new(nebula_app));
    ...
    let app_for_timer = Rc::clone(&nebula_app);
