//! Live prototype for Cascade. Same pattern as slint_plaits_live.rs --
//! see slint_mixer_live.rs's doc comment for the fuller "why
//! `#[path]`" explanation. Drives the real `CascadeApp` (a 16-pad FM
//! keyboard, fully polyphonic) through Slint: pads trigger real notes,
//! left knob scrolls/expands the real menu, right knob edits the
//! selected row. Includes the real arpeggiator (see arpeggiator.rs)
//! -- browse to "Arp" and turn it on to hear held pads arpeggiate
//! instead of sounding as a chord.
//!
//! Run it:
//!     cargo run --example slint_cascade_live
...
slint::slint! {
    ...
    export component LiveCascadeScreen inherits DeviceFrame {
        app-name: "CASCADE";
...
fn main() {
    ...
    let mut cascade_app = CascadeApp::new(Arc::clone(&sensitivity), Arc::clone(&nav_speed), Arc::clone(&modbus), Arc::clone(&audio_bus), Arc::clone(&mixer_bus));
    if let Some(p) = cascade_app.audio_processor() {
        engine.add(p);
    }
    cascade_app.on_enter();

    let (audio_host, device_state) = AudioHost::open_default(Arc::clone(&engine)).expect("open default audio output");
    println!("Live Cascade prototype -- output device: {}", device_state.current_output());
    let _audio_host = audio_host;

    let ui = LiveCascadeScreen::new().unwrap();
    let cascade_app = Rc::new(RefCell::new(cascade_app));
    ...
    let app_for_timer = Rc::clone(&cascade_app);
